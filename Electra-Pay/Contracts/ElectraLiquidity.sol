 ...
 * @notice Manages liquidity provision for idle funds in the ElectraPay system
...
contract ElectraLiquidity is Ownable, ReentrancyGuard {
    ...
    constructor(address initialOwner, address _payrollContract) Ownable(initialOwner) ReentrancyGuard() {
        ...
        _createPool("usdc-eth", "USDC-ETH", 820, 1); // 8.20% APY, Low risk
        _createPool("wbtc-eth", "WBTC-ETH", 650, 1); // 6.50% APY, Low risk
        _createPool("usdt-eth", "USDT-ETH", 780, 1); // 7.80% APY, Low risk
        _createPool("dai-usdc", "DAI-USDC", 520, 1); // 5.20% APY, Low risk
        _createPool("sblz-eth", "SBLZ-ETH", 1240, 2); // 12.40% APY, Medium risk
    }
     ...
     */
    function calculatePendingRewards(string calldata _positionId) public view returns (uint256 pendingRewards) {
        Position storage position = positions[_positionId];
    ...
    receive() external payable {
        // Just receive ETH, no specific action needed
    }
}